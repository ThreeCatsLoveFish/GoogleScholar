...
returned results. It processes all results pages, supports recursive
crawler.
"""
import json
import optparse
...
class ScholarArticle(object):
    ...
    def as_citation(self):
        ...
        """
        return self.citation_data or self.as_txt()
...
class ScholarQuerier(object):
    ...
    # Default URLs for getting citation link:
    GET_CITATION_URL = ScholarConf.SCHOLAR_SITE + '/scholar?' \
    ...
    def __init__(self):
        ...
        if ScholarConf.CONFIG_FILE and \
                os.path.exists(ScholarConf.CONFIG_FILE):
            with open(ScholarConf.CONFIG_FILE, 'r') as f:
                self.config = json.load(f)
                ScholarUtils.log('info', 'retrieving citation export data {self.config}')
    ...
    def get_paper(self, query):
        ...
        if html is None:
            return
        self.parse(html)
        while len(self.articles) < ScholarConf.MAX_PAGE_RESULTS and len(self.articles) != article_num:
            article_num = len(self.articles)
            # this is a workaround to fetch all the paper, ought to be better integrated at some point
            time.sleep(sleep_time)
            html = self._get_http_response(url=query.get_url() + '&start=' + str(len(self.articles)),
                                           log_msg='dump of query response HTML',
                                           err_msg='results retrieval failed')
            if html is None:
                return
    ...
    def get_citations(self, query):
        ...
        if html is None:
            return
        self.parse(html)
        while len(self.articles) < citations_num and len(self.articles) != article_num:
    ...
    def get_citation_data(self, article):
        ...
        """
        if self.settings is None or not self.settings.is_configured():
            return False
        if article['aid'] is None:
            return False
        if article.citation_data is not None:
            return True
        
        urlargs = {'aid': article['aid']}
        html = self._get_http_response(url=self.GET_CITATION_URL % urlargs,
        ...
        article['url_citation'] = re.sub(r'scisf:%d', 'scisf:' + str(self.settings.citform), tag[0]['href'])
    ...
    def _get_http_response(self, url, log_msg=None, err_msg=None):
        ...
        try:
            ...
            ScholarUtils.log('debug', 'Url: %s' % resp.url)
            ScholarUtils.log('debug', 'Status: %s' % resp.status_code)
...
def txt(querier, with_globals, filename=None):
    if with_globals:
    ...
    if len(articles) > 0 and filename:
        file = open(filename, 'w', encoding='utf-8')
    for art in articles:
        print(encode(art.as_txt()) + '\n')
        if filename:
            file.write(encode(art.as_txt()) + '\n')
...
def citation_export(querier, filename=None):
    articles = querier.articles
    citations_num = querier.citations_num
    if len(articles) > 0 and filename:
        filename = filename[:-4] + f"_{len(articles)}-{citations_num}" + filename[-4:]
        file = open(filename, 'w', encoding='utf-8')
    for art in articles:
        print(encode(art.as_txt()) + '\n')
...
scholar.py -c 5 -a "albert einstein" -t --none "quantum theory" --after 1970"""
    ...
    group.add_option('-o', '--output-file', metavar='OUTPUT', default=None,
                     help='Output file name(s)')
    parser.add_option_group(group)

    group = optparse.OptionGroup(parser, 'Miscellaneous')
    group.add_option('--config-file', metavar='FILE', default=None,
                     help='File to use for config storage. If given, will read any existing config if found at startup.')
    ...
    if options.config_file:
        ScholarConf.CONFIG_FILE = options.config_file
    ...
    if options.count is not None:
        options.count = max(options.count, ScholarConf.MAX_PAGE_RESULTS)
        query.set_num_page_results(options.count)

    if options.citations_only:
        querier.get_citations(query)
    else:
        querier.get_paper(query)
    ...
    if options.csv:
        ...
        citation_export(querier, filename=filename)
    else:
        txt(querier, with_globals=options.txt_globals, filename=filename)
